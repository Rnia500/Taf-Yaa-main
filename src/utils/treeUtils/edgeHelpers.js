...
export function createPolygamousEdge(source, target, options = {}) {
  return {
    id: `edge-${source}-${target}`,
    source,
    target,
...
export function createEdgeWithGuard(edgeCreator, nodesMap, ...args) {
  ...
  if (!nodesMap.has(edge.source) || !nodesMap.has(edge.target)) {
    console.warn(`Skipping edge creation: source "${edge.source}" or target "${edge.target}" not found in nodesMap`);
    return null;
  }
